...
favoritesRouter.get('/', async (req: AuthedRequest, res) => {
  const r = await query<{ attraction_id: string; created_at: Date }>(
    'SELECT attraction_id, created_at FROM favorites WHERE user_id = $1 ORDER BY created_at DESC',
    [req.user!.id],
  );
  res.json({ favorites: r.rows.map((row) => ({ attractionId: row.attraction_id, createdAt: row.created_at })) });
});
